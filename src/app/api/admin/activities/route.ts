...
export async function GET(req: NextRequest) {
  try {
    ...
    console.error("[DEBUG activities GET]", e?.stack || e);
    return NextResponse.json({ error: "DEBUG: " + (e?.message || String(e)) }, { status: 500 });
  }
}
...
export async function POST(req: NextRequest) {
  ...
  return NextResponse.json({ activityId: activityId, name: name }, { status: 201 });
    ...
    console.error("[DEBUG activities POST]", e?.stack || e);
    return NextResponse.json({ error: "DEBUG: " + (e?.message || String(e)) }, { status: 500 });
  }
}
