...
function evaluateCompleted(
  ...
  return required.every(function (feature) {
    ...
    if (feature === "videoSubmission") return justSubmittedVideo;
    return false;
  });
}
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const storageKey = "video/" + item.assignedItemId + "/" + fileId;
...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (!submission) {
    return NextResponse.json({ error: "제출된 영상 파일이 없습니다." }, { status: 404 });
  }
